...
def metric_barplots(
    ...
    metrics = {
        ...
        'mean_event_size_pct':   'Mean event size (%)',
    }
    ...
    # fixed y-axis upper limit
    fixed_ymax = {
        'pct_active_neurons': 100,
        'synchrony_index':    1,
        'mean_event_size_pct': 100,
    }
...
# preview of network events with raster + population activity overlay, for quick per-recording QC and threshold tuning
...
def network_event_overlay_preview(
    ...
    ax_pop      = fig.add_subplot(gs[1, 0], sharex=ax_raster)
    ...
    for ax in (ax_raster, ax_pop, ax_rate_bar):
...
# main entry point to generate all plots
